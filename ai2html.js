...
function main() {
    ...
    forEach(doc.placedItems, function (item) {
      ...
      if (!/\/ai\//.test(path)) {
        error(
          'A placed image (' +
            name +
            ') needs to be moved inside the ai/ folder of the project'
        );
      }
    });
  }
  ...
  function readConfigFileSettings() {
    ...
    var globalSettings =
      fileExists(globalPath) ? readSettingsFile(globalPath) : {};
    var localSettings =
      fileExists(localPath) ? readSettingsFile(localPath) : {};
  ...
  function exportSymbolAsHtml(item, geometries, abBox, opts) {
    ...
    var properties =
      item.name ? 'data-name="' + makeKeyword(item.name) + '" ' : '';
    var geom, x, y;
  ...
  function convertArtItems(activeArtboard, textFrames, masks, settings) {
    ...
    forEachImageLayer('png', function (lyr) {
      ...
      var fmt =
        contains(settings.image_format || [], 'png24') ? 'png24' : 'png';
  ...
  function getResizerScript(containerId) {
    ...
    var resizer = function (containerId, opts) {
      ...
      function findContainers(id) {
        ...
        return selectChildren('.ai2html-responsive', document).filter(
          function (el) {
